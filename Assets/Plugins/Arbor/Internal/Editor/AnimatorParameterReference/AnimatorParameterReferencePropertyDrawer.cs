...
namespace ArborEditor
{
	...
	internal sealed class AnimatorParameterReferencePropertyDrawer : PropertyDrawer
	{
		Rect DoGUI(Rect position, SerializedProperty property, GUIContent label, bool isDraw)
		{
			...
			lineRect.height = AnimatorGUITools.GetAnimatorParameterFieldHeight(animator, nameProperty, typeProperty, hasType);

			if (isDraw)
			{
				AnimatorGUITools.AnimatorParameterField(lineRect, animator, nameProperty, typeProperty, EditorGUITools.GetTextContent("Parameter"), hasType, parameterType);
			}
